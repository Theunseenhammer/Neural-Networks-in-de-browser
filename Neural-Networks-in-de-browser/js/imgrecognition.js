...
const updateInstruction = () => {
    if (typeof inputCount[currentInput] === "undefined") {
        startPredict = true;
        instruction.innerText = `Teken een van de nummers die je al hebt getekend`;
        return;
    }

    const text = `Teken ${currentInput}`;
    instruction.innerText = instruction.innerText === text ? text + " opnieuw!" : text;
}
...
const predict = (testData) => {
    ...
    net.train(userInput, { iterations: 10000, log: true });
